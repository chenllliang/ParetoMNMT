...
class MultilingualDatasetManager(object):
    def __init__(self, args, lang_pairs, langs, dicts, sampling_method):
        ...
        self.first_sort_by_source = self.args.first_sort_by_source
    ...
    def add_args(parser):
        parser.add_argument(
            ...
            default=None,
            type=str,
            help="recipe for different directions of the hotpot, like __fr_XX__:20000,__de_DE__:2000,"
            "specify how many tokens in one batch belong to certain direction",
        )
    ...
    def load_into_concat_dataset(self, split, datasets, data_param_list):
        if self.args.lang_tok_replacing_bos_eos:
            ...
            return SampledMultiDataset(
                ...
                virtual_size=None,
                split=split,
                first_sort_by_source=self.first_sort_by_source
            )
    ...
    def load_original_multi_dataset(
        self, split, training, epoch=0, combine=False, shard_epoch=None, **kwargs
    ):
        datasets, data_param_list = self.load_split_datasets(
                split, training, epoch, combine, shard_epoch=shard_epoch, **kwargs
        )
        assert len(eval(self.args.recipe))>0
        
        if training and split == getattr(self.args, "train_subset", None):
            datasets_for_each_direction = {
               i:HotPotMultiDataset(
                OrderedDict([(i,j)]),
                sampling_ratios=None,
                eval_key=None,
                collate_format=CollateFormat.single,
            ...
            return HotPotMultiDatasetCombine(datasets_for_each_direction)
    ...
    def load_dataset(
        ...
        return self.load_original_multi_dataset(
