# a fairseq dataset that supports multiple languages with fixed number of tokens per language in a batch
...
class HotPotMultiDatasetCombine(FairseqDataset):
    def __init__(self,datasets:dict):
        self.datasets = datasets
        self.directions = list(datasets.keys())





class HotPotMultiDataset(FairseqDataset):
    ...
    def __init__(
        ...
        shuffle=True,
        first_sort_by_source=False,
        recipe = None
    ):
        super().__init__()
        self.shared_collater = shared_collater
        self.shuffle = shuffle
        self.first_sort_by_source = first_sort_by_source
    ...
    def random_choice_in_dataset(self, rng, dataset, choice_size):
        ...
        # replace means whether it can sample a data item more than once
    ...
    def get_virtual_indices(self, rng, datasets, sample_ratios, virtual_size):
        ...
        if sample_ratios is None:
            ...
            ratios = sample_ratios / sample_ratios.sum() # sum is 1
        ...
        cumulative_sizes = np.cumsum(virtual_sizes_per_dataset) # 累积和
    ...
    def ordered_indices(self):
        ...
        if self.first_sort_by_source:
            if src_sizes is not None:
                indices = indices[np.argsort(src_sizes[indices], kind="mergesort")]
            
            sort_indices = indices[np.argsort(tgt_sizes[indices], kind="mergesort")]
