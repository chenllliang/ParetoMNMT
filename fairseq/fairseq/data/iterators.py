...
def merge_dataset_with_mapping(datasets:dict):
    dataset_list = list(datasets.values())
    
    for i in range(len(dataset_list)):
        if i == 0:
            concat_dataset = dataset_list[i]
        else:
            concat_dataset += dataset_list[i]
    ...
    for i,j in datasets.items():
        current_mapping = {}
        for c_index in range(len(j)):
            current_mapping[c_index] = current_index
            current_index += 1
        mapping[i] = current_mapping
    
    return concat_dataset, mapping
        
    


class HotpotIterator(EpochBatchIterating):
    def __init__(
        self,
        datasets,
        collate_fn,
        ...
        dataset, data_mapping = merge_dataset_with_mapping(datasets)
        ...
        # create sample(single dir)2sample_id(total dataset) mapping
    ...
    def combine_batches(self):
        # combine batches from different directions
        all_batches = {}
        for i,j in self.datasets.items():
            all_batches[i] = tuple(self.batch_samplers[i](j,self.epoch))
        
        max_batches_length = max([len(i) for i in all_batches.values()])
        
        # update index in all_batches using data_mapping
        mapped_batches = {}
        
        for direction,batches in all_batches.items():
            current_batch_indexes = []
            for i in batches:
                index2list  = i.tolist()
                mapped_indexes = [self.data_mapping[direction][i] for i in index2list]
                current_batch_indexes.append(mapped_indexes)
            mapped_batches[direction] = tuple(current_batch_indexes)
        
        final_batches = []
        batches_length_each_direction = {i:len(j) for i,j in mapped_batches.items()}
        
        for i in range(max_batches_length):
            current_batch = []
            for direction in mapped_batches.keys():
                # add data cyclicly
                current_batch += mapped_batches[direction][i%batches_length_each_direction[direction]]
            final_batches.append(np.array(current_batch))
        
        final_batches = tuple(final_batches)
        return final_batches
        
    @property
    def frozen_batches(self):
        if self._frozen_batches is None:
            self._frozen_batches = self.combine_batches()
    ...
    def next_epoch_itr(
        ...
        if self._next_epoch_itr is not None:
            ...
            if callable(list(self.batch_samplers.values())[0]) and prev_epoch != self.epoch:
...
class HotpotEpochBatchIterator(EpochBatchIterating):
    def __init__(
        self,
        iters:dict,
        recipe=None,
        ...
        assert recipe != None
        self.iters_each_direction = iters
        self.epoch = epoch
    ...
    def combine_batches(self):
        # combine batches from different directions
        all_batches = {}
        for i,j in self.iters_each_direction.items():
            all_batches[i] = tuple(j.batch_sampler(j.dataset,j.epoch))
        
        max_batches_length = max([len(i) for i in all_batches.values()])
        
        final_batches = tuple()
        
        # all batches from different directions needed to be trained in a epoch
    ...
    def frozen_batches(self):
        if self._frozen_batches is None:
            ...
            self._frozen_batches = all_batches
            self.frozen_batches = all_batches
