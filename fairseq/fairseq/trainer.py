...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop, gradient accumulation
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        if self.task.second_d:
            sample, is_dummy_batch = self._prepare_sample(sample)
            self.task.per_task_valid_step_sharpness(sample, self.model, self.criterion, **extra_kwargs)
            self.zero_grad()
        ...
        with torch.no_grad():
            ...
            try:
                ...
                #HACK
                if self.task.is_curriculum or self.task.log_task_valid_loss:
                     self.task.per_task_valid_step(
                    sample, self.model, self.criterion, **extra_kwargs
                )
